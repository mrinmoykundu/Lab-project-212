classdef QMSimulatorApp < handle
    %
    % Usage:
    %     app.QMSimulatorApp
    ...
    properties (Constant)
        AppPathBase = {'/QMSimulator/code'};
        AppClass = 'QMSimulatorApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            QMSimulatorApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'project1')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong project1
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'project1').getString, ...
            ...
            if nargout(@project1) == 0
                eval('project1');
            else
                obj.AppHandle = eval('project1');
            end
